...
# -------------------------------------------------------------------------------------------------------------------------------
# Stage 5: Survival Analysis & Currency Modelling                                                                               
# -------------------------------------------------------------------------------------------------------------------------------
# Fits parametric survival functions to labelled topic data to estimate currency decay.                                          
# For each topic: prepares data → exhaustive search across 10 distributions × 199 bin sizes →                                   
# ranks by average of log-loss, AUC, AIC, BIC → selects best model → plots empirical vs fitted.                                
#                                                                                                                               
# Distributions: Exponential, Weibull, Exp-Weibull, Piecewise-Exp, Logistic,                                                    
#                LogNormal, Gamma, Generalised Gamma, Burr XII, Burr III                                                        
#                                                                                                                               
# Input: data_labelling/topic_{id}/topic_labeled_updated.csv (from Stage 4, relative to script dir)                             
# Output (saved to outputs/survival_analysis/):                                                                                 
#   - results_topic_{id}.csv — all model fits ranked                                                                            
#   - plot_topic_{id}.png — empirical vs best fitted curve                                                                      
#   - currency_model_summary.csv — best model per topic                                                                         
#                                                                                                                               
# This stage triggered escalation during pipeline execution (Doer failed to produce                                       
# a working script within retry limits). Written manually as human-corrected implementation.                                    
# -------------------------------------------------------------------------------------------------------------------------------
...
def process_topic(topic_df, topic_id):
    ...
    topic_df = topic_df[topic_df["final_label"].notna()].copy()
    # Convert to int
    ...
    # Save full results
    results_df.to_csv(OUTPUT_DIR / f"results_{topic_name}.csv", index=False)
    plt.savefig(OUTPUT_DIR / f"plot_{topic_name}.png", dpi=150)
...
# 1. This finds the exact folder where this script file is saved
SCRIPT_DIR = Path(__file__).resolve().parent
OUTPUT_DIR = SCRIPT_DIR / "outputs" / "survival_analysis"
...
def main():
    ...
    for topic_id in topic_ids:
        # 2. This creates an absolute path starting from your script's folder
        filepath = SCRIPT_DIR / "outputs" / "data_labelling" / f"topic_{topic_id}" / "topic_labeled_updated.csv"

        # 3. Pathlib objects work perfectly with os.path.exists
    ...
    if summary:
        ...
        # 4. Saves the summary CSV into the same folder as the script
        output_path = OUTPUT_DIR / "currency_model_summary.csv"
